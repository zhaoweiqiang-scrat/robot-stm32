...
USART_TypeDef*        SERIAL_USART[SERIALn] = {RIKI_SERIAL1, RIKI_SERIAL2, RIKI_SERIAL3, RIKI_SERIAL4};
GPIO_TypeDef*         SERIAL_PORT[SERIALn] = {RIKI_SERIAL1_GPIO_PORT, RIKI_SERIAL2_GPIO_PORT, RIKI_SERIAL3_GPIO_PORT, RIKI_SERIAL4_GPIO_PORT}; 
const uint32_t        SERIAL_USART_CLK[SERIALn] = {RIKI_SERIAL1_CLK, RIKI_SERIAL2_CLK, RIKI_SERIAL3_CLK, RIKI_SERIAL4_CLK};
const uint32_t        SERIAL_PORT_CLK[SERIALn] = {RIKI_SERIAL1_GPIO_CLK, RIKI_SERIAL2_GPIO_CLK, RIKI_SERIAL3_GPIO_CLK, RIKI_SERIAL4_GPIO_CLK};
const uint32_t        SERIAL_DMA_CLK[SERIALn] = {RIKI_SERIAL1_DMA_CLK,RIKI_SERIAL2_DMA_CLK,RIKI_SERIAL3_DMA_CLK,RIKI_SERIAL4_DMA_CLK};
const uint16_t        SERIAL_TX_PIN[SERIALn] = {RIKI_SERIAL1_TX_PIN, RIKI_SERIAL2_TX_PIN, RIKI_SERIAL3_TX_PIN, RIKI_SERIAL4_TX_PIN}; 
const uint16_t        SERIAL_RX_PIN[SERIALn] = {RIKI_SERIAL1_RX_PIN, RIKI_SERIAL2_RX_PIN, RIKI_SERIAL3_RX_PIN, RIKI_SERIAL4_RX_PIN}; 
const uint16_t        SERIAL_IRQn[SERIALn] = {RIKI_SERIAL1_IRQ, RIKI_SERIAL2_IRQ, RIKI_SERIAL3_IRQ, RIKI_SERIAL4_IRQ};
const uint16_t        SERILA_NVIC[SERIALn] = {RIKI_SERIAL1_NVIC, RIKI_SERIAL2_NVIC, RIKI_SERIAL3_NVIC, RIKI_SERIAL4_NVIC};
DMA_Stream_TypeDef *  SERILA_TX_DMA[SERIALn] = {RIKI_SERIAL1_DMA, RIKI_SERIAL2_DMA, RIKI_SERIAL3_DMA, RIKI_SERIAL4_DMA};
const uint32_t        SERILA_TX_DMA_CH[SERIALn] = {RIKI_SERIAL1_DMA_CH,RIKI_SERIAL2_DMA_CH,RIKI_SERIAL3_DMA_CH,RIKI_SERIAL4_DMA_CH}; 
const uint32_t       SERILA_TX_DMA_ADDR[SERIALn] = {RIKI_SERIAL1_DMA_ADDR,RIKI_SERIAL2_DMA_ADDR,RIKI_SERIAL3_DMA_ADDR,RIKI_SERIAL4_DMA_ADDR};    
const uint32_t        SERILA_TX_DMA_TCIF[SERIALn] = {RIKI_SERIAL1_DMA_TCIF,RIKI_SERIAL2_DMA_TCIF,RIKI_SERIAL3_DMA_TCIF,RIKI_SERIAL4_DMA_TCIF};
const uint8_t         SERILA_GPIO_AF[SERIALn] = {RIKI_SERIAL1_GPIO_AF,RIKI_SERIAL2_GPIO_AF,RIKI_SERIAL3_GPIO_AF,RIKI_SERIAL4_GPIO_AF};
const uint8_t		  SERILA_TX_GPIO_SOURCE[SERIALn] = {RIKI_SERIAL1_TX_GPIO_Source,RIKI_SERIAL2_TX_GPIO_Source,RIKI_SERIAL3_TX_GPIO_Source,RIKI_SERIAL4_TX_GPIO_Source};
const uint8_t		  SERILA_RX_GPIO_SOURCE[SERIALn] = {RIKI_SERIAL1_RX_GPIO_Source,RIKI_SERIAL2_RX_GPIO_Source,RIKI_SERIAL3_RX_GPIO_Source,RIKI_SERIAL4_RX_GPIO_Source};
uint8_t USE_LEN;
...
void HardwareSerial::begin(uint32_t baud)
{
  ...
  DMA_InitStructure.DMA_PeripheralBaseAddr = SERILA_TX_DMA_ADDR[this->Serial];//DMA外设地址
  ...
  DMA_Init(SERILA_TX_DMA[this->Serial], &DMA_InitStructure);//初始化DMA Stream
	...
	USART_DMACmd(SERIAL_USART[this->Serial],USART_DMAReq_Tx,ENABLE);  //使能串口的DMA发送  	
...
// if the head isn't ahead of the tail, we don't have any characters
  ...
  //uint8_t uc = rx_buffer._aucBuffer[rx_buffer._iTail] ;
	uint8_t uc = USART_RX_BUF[USE_LEN++];
	if(USE_LEN >= USART_RX_STA)
	{
		DMA_RX2_FLAG = 0;
		USE_LEN=0;
	}
...
uint8_t usart2_buf[20],usart_len,Msg_Len;
void HardwareSerial::irq()
{
	uint8_t data,First_Data;
	if (USART_GetITStatus(SERIAL_USART[Serial], USART_IT_RXNE) != RESET) {
		data = USART_ReceiveData(SERIAL_USART[Serial]);
		usart2_buf[usart_len++] = data;
		if(usart2_buf[0] != 0xff)
		{
			usart_len = 0;
		}
		else if((usart_len==2)&&(usart2_buf[1] != 0xfe))
		{
			usart_len = 0;
		}
		else
		{
			...
			if(usart_len>=8)
			{
				Msg_Len = usart2_buf[3]<<8|usart2_buf[2]+8;
			  if(usart_len >= Msg_Len)usart_len=0;
			}
