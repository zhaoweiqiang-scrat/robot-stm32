...
{
	char ultr_distance_buffer[]= "The distance was not updated! ";
	
	if(!Sensors_Ultrasonic_task())
	{
		raw_ultrasonic_msg.ultr_distance = Ultr_distance;
		raw_ultrasonic_pub.publish(&raw_ultrasonic_msg);
	}
	else
	{
		nh.logwarn(ultr_distance_buffer);
	}
